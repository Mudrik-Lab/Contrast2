...
class UnConSuppressedStimulus(models.Model):
    ...
    soa = models.DecimalField(
        null=False, blank=False, max_digits=10, decimal_places=3
    )  # ms Stimulus Onset Asynchrony (which is the time that passes from the onset of the stimulus to the onset of the next stimulus)
...
class UnConTargetStimulus(models.Model):
    ...
    suppressed_stimulus = models.ForeignKey(
        null=False, blank=False, on_delete=CASCADE, to=UnConSuppressedStimulus, related_name="target_stimuli"
    )
