...
AggregatedInterpretationDTO = namedtuple("AggregatedInterpretationDTO",
                                         ["parent_theory_names", "parent_theory_acronyms", "type"])
...
class AggregatedInterpretationService:
    ...
    def resolve(self) -> List[AggregatedInterpretationDTO]:
        ...
        if len(pro_names):
            pro_acronym_names = [x.parent_theory_acronyms for x in self.interpretations if
                                 x.type == InterpretationsChoices.PRO]
        ...
        challenge_names = [x.parent_theory_names for x in self.interpretations if
                           x.type == InterpretationsChoices.CHALLENGES]
        if len(challenge_names):
            challenges_acronym_names = [x.parent_theory_acronyms for x in self.interpretations if
                                        x.type == InterpretationsChoices.CHALLENGES]
