...
class JournalsGraphTestCase(BaseTestCase):
    ...
    def _given_world_setup(self):
        self.israeli_study = self.given_study_exists(
            title="Israeli study",
            countries=["IL"],
        ...
        self.british_israeli_study = self.given_study_exists(
            title="british",
            countries=["UK", "IL"],
        ...
        british_israeli_study_experiment = self.given_experiment_exists_for_study(
            study=self.british_israeli_study,
        ...
        self.given_interpretation_exist(
            experiment=israeli_study_experiment, theory=gnw_child_theory, interpretation_type=InterpretationsChoices.PRO
        )
    ...
    def test_journals_by_theory(self):
        ...
        target_url = self.reverse_with_query_params("experiments-graphs-journals", theory=self.gnw_parent_theory.id)
        ...
        target_url = self.reverse_with_query_params("experiments-graphs-journals", theory=self.rpt_parent_theory.id)
        res = self.client.get(target_url)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(len(res.data), 1)  # as only one study with this interpretation as pro
        first_result = res.data[0]
    ...
    def test_journals_by_theory_family_data(self):
        ...
        self.given_experiment_exists_for_study(
            study=israeli_study, results_summary="brave new world", is_reporting=ReportingChoices.NO_REPORT
        )
