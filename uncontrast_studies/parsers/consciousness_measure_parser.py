...
from configuration.uncontrast_initial_data_types import uncon_consciousness_measures_types, \
...
UnConResolvedConMeasure = namedtuple("UnConResolvedConMeasure", ["phase", "type", "sub_type",
                                                                 "number_of_trials", "number_of_awareness_participants",
                                                                 "is_cm_pax_same_as_task", "is_performance_above_chance",
                                                                 "is_trial_excluded_based_on_measure", "notes"])
...
def resolve_consciousness_measures(item: dict, index: str):
    ...
    is_same_as_task_data = item["Consciousness Measures Is the measure taken from the same participants as the main task?"]
    ...
    is_trials_excluded_data = item["Consciousness Measures Were trials excluded from the analysis based on the measure?"]
    ...
    for resolved_type in main_types:
        if resolved_type not in uncon_consciousness_measures_types.keys():
            raise InvalidConsciousnessMeasureDataError(f"invalid consciousness measure main type {resolved_type}, index {index}")
        else:
            ...
            if len(uncon_consciousness_measures_types[main_type]) == 0 and resolved_cm_specific_type in NULL_VALUES:
                ...
                raise InvalidConsciousnessMeasureDataError(f"missing CM specific type for main type {main_type}, index {index}")
            elif resolved_cm_specific_type in uncon_consciousness_measures_types[main_type]:
                specific_type = resolved_cm_specific_type
            else:
                raise InvalidConsciousnessMeasureDataError(f"invalid CM specific type {resolved_cm_specific_type} for main type {main_type}, index {index}")

        resolved_cm_phase = str(cm_phase_data).strip()
        if resolved_cm_phase not in uncon_consciousness_measures_phases:
            raise InvalidConsciousnessMeasureDataError(f"invalid consciousness measure phase {resolved_cm_phase}, index {index}")
        else:
        ...
        if number_of_trials_objective_data in NULL_VALUES:
            ...
            try:
                ...
                raise InvalidConsciousnessMeasureDataError(f"invalid numeric data for number of trials (column AD), index {index}")
        ...
        if number_of_awareness_participants_data in NULL_VALUES:
            ...
            try:
                ...
                raise InvalidConsciousnessMeasureDataError(f"invalid numeric data for number of awareness participants (column AF), index {index}")
        ...
        if str(is_same_as_task_data) in NULL_VALUES:
            ...
            raise InvalidConsciousnessMeasureDataError(f"invalid boolean data for is_same_as_task (column AE), index {index}")
        ...
        if str(is_above_chance_data) in NULL_VALUES:
            ...
            raise InvalidConsciousnessMeasureDataError(f"invalid boolean data for is_performance_above_chance (column AG), index {index}")
        ...
        if str(is_trials_excluded_data) in NULL_VALUES:
            ...
            raise InvalidConsciousnessMeasureDataError(f"invalid boolean data for were_trials_excluded (column AH), index {index}")

        resolved_consciousness_measures.append(UnConResolvedConMeasure(type=main_type, phase=phase, sub_type=specific_type,
