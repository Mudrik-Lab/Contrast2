...
class ParametersDistributionFreeQueriesDataProcessor(BaseProcessor):
    ...
    def get_queryset(self):
        ...
        if len(self.interpretations):
            pass
            # TODO add filtering by interpretations if chosen
        if self.theories:
            # Find out about the connection between this and theories
            pass
        ...
        if len(self.populations):
            queryset = queryset.filter(samples__type__name__in=self.populations)
    ...
    def _aggregate_query_by_breakdown(self, queryset: QuerySet, filtered_subquery: QuerySet):
        ...
        ids_subquery = annotated_subquery \
            .order_by("experiment") \
