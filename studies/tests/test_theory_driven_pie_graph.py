...
from contrast_api.choices import ReportingChoices, InterpretationsChoices, TheoryDrivenChoices
...
class TheoryDrivenPieGraphTestCase(BaseTestCase):
    ...
    def _given_world_setup(self):
        ...
        self.given_interpretation_exist(
            experiment=israeli_study_experiment,  # mentioning
        ...
        self.given_interpretation_exist(
            experiment=israeli_study_experiment_2,  # driven
            theory=self.gnw_child_theory,
            interpretation_type=InterpretationsChoices.PRO,
        )
    ...
    def test_theory_driven(self):
        ...
        target_url = self.reverse_with_query_params(
            "experiments-graphs-theory-driven-distribution-pie", interpretation="pro"
        )
        ...
        target_url = self.reverse_with_query_params(
            "experiments-graphs-theory-driven-distribution-pie", interpretation=InterpretationsChoices.CHALLENGES
        )
        res = self.client.get(target_url)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
