...
class FullUncontrastExperimentResource(resources.ModelResource):
    ...
    def dehydrate_suppression_methods(self, experiment: UnConExperiment):
        return SEPERATOR.join(f"{method.type.name} - {method.sub_type.name if method.sub_type is not None else None}" for method in experiment.suppression_methods.all())
    def dehydrate_finding(self, experiment: UnConExperiment):
        return SEPERATOR.join(
            self.resolve_finding_tag_description(finding) for finding in experiment.findings.all()
        )
