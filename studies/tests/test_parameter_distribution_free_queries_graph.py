...
class ParameterDistributionFreeQueriesGraphTestCase(BaseTestCase):
    def setUp(self) -> None:
        super().setUp()
    ...
    def test_parameters_distribution_breakdown_paradigm_family(self):
        (
            another_different_child_paradigm,
            different_child_paradigm,
            different_parent_paradigm,
        ...
        target_url = self.reverse_with_query_params(
            "experiments-graphs-parameters-distribution-free-queries", breakdown="paradigm_family"
        )
        ...
        self.assertEqual(first_series["key"], masking_parent_paradigm.name)  # 3 total
        ...
        target_url = self.reverse_with_query_params(
            "experiments-graphs-parameters-distribution-free-queries",
            breakdown="paradigm_family",
            techniques=[first_technique.id],
        )
        ...
        first_series = res.data[0]
        second_series = res.data[1]
        self.assertEqual(first_series["key"], different_parent_paradigm.name)
    ...
    def test_sample_breakdown(self):
        ...
        target_url = self.reverse_with_query_params(
            "experiments-graphs-parameters-distribution-free-queries", breakdown="population"
        )
        res = self.client.get(target_url)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(len(res.data), 2)  # as gnw child interpretation to two paradigms
    ...
    def test_all_breakdown_sanity_test(self):
        ...
        for breakdown in BREAKDOWN_OPTIONS:
            target_url = self.reverse_with_query_params(
                "experiments-graphs-parameters-distribution-free-queries", breakdown=breakdown
            )
            res = self.client.get(target_url)
            self.assertEqual(res.status_code, status.HTTP_200_OK)
