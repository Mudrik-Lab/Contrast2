...
class StudyRelatedPermissionsViewMixin:
    def initial(self, request, *args, **kwargs):
        self.parent_object = get_object_or_404(
            Study.objects.filter(approval_status=ApprovalChoices.PENDING), id=self.kwargs["study_pk"]
        )
...
class ExperimentRelatedNestedObjectMixin:
    ...
    def get_queryset(self):
        qs = super().get_queryset().filter(experiment_id=self.kwargs.get("experiment_pk"))
        return qs
