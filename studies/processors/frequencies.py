...
class FrequenciesGraphDataProcessor(BaseProcessor):
    ...
    def process(self):
        ...
        relevant_finding_tags = (
            FindingTag.objects.select_related("experiment")
        ...
        order_query = relevant_finding_tags.filter(experiment=OuterRef("experiment_id")).order_by("band_lower_bound")
        max_order_query = relevant_finding_tags.filter(experiment=OuterRef("experiment_id")).order_by(
            "band_higher_bound"
        )

        qs = (
            experiments.annotate(min_band_lower=Subquery(order_query.values("band_lower_bound")[:1]))
