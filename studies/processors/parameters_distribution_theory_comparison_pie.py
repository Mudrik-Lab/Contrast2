...
class ComparisonParametersDistributionPieGraphDataProcessor(BaseProcessor):
    ...
    def process_population(self, theory_interpretations_experiments):
        subquery = (
            Sample.objects.filter(experiment__in=theory_interpretations_experiments)
            .distinct()
            .values("type")
    ...
    def process_reporting(self, theory_interpretations_experiments):
        subquery = (
            Experiment.objects.filter(id__in=theory_interpretations_experiments)
            .distinct()
            .values("is_reporting")
    ...
    def process_task(self, theory_interpretations_experiments):
        subquery = (
            TaskType.objects.filter(task__experiment__in=theory_interpretations_experiments)
            .distinct()
            .values("name")
    ...
    def process_stimuli_category(self, theory_interpretations_experiments):
        subquery = (
            StimulusCategory.objects.filter(stimuli__experiment__in=theory_interpretations_experiments)
            .distinct()
            .values("name")
    ...
    def process_modality(self, theory_interpretations_experiments):
        subquery = (
            ModalityType.objects.filter(stimuli__experiment__in=theory_interpretations_experiments)
            .distinct()
            .values("name")
            .annotate(experiment_count=Count("stimuli__experiment", distinct=True))
            .annotate(key=F("name"))
    ...
    def process_consciousness_measure_phase(self, theory_interpretations_experiments):
        subquery = (
            ConsciousnessMeasurePhaseType.objects.filter(
                consciousness_measures__experiment__in=theory_interpretations_experiments
            )
            .distinct()
            .values("name")
    ...
    def process_consciousness_measure_type(self, theory_interpretations_experiments):
        subquery = (
            ConsciousnessMeasureType.objects.filter(
                consciousness_measures__experiment__in=theory_interpretations_experiments
            )
            .distinct()
            .values("name")
    ...
    def process_technique(self, theory_interpretations_experiments):
        subquery = (
            Technique.objects.filter(experiments__in=theory_interpretations_experiments)
            .distinct()
            .values("name")
            .annotate(experiment_count=Count("experiments", distinct=True))
            .annotate(key=F("name"))
    ...
    def process_measure(self, theory_interpretations_experiments):
        subquery = (
            MeasureType.objects.filter(measures__experiment__in=theory_interpretations_experiments)
            .distinct()
            .values("name")
