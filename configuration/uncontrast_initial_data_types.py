uncon_paradigms = {
    "Priming": [
        "Semantic",
        "Affective",
        "Perceptual",
        "Action",
    ],
    "Emergence to awareness": ["B-CFS", "Mooney images"],
    "Attention allocation": [],
    "Change blindness": [],
    "Learning": [
        "Instrumental conditioning",
        "Evaluative conditioning",
        "Eyeblink conditioning",
        "Conflict adaptation",
        "Associative learning",
    ],
    "Cognitive Tasks": ["Executive control", "Working memory", "Task switching", "Memory", "Language"],
    "Computational modeling": [],
    "Illusions": [],
    "Sperling like": [],
    "Visual search": [],
}

uncon_tasks = [
    "Numerical categorization",
    "Lexical categorization",
    "Semantic categorization",
    "Perceptual categorization",
    "Go/No go",
    "Search",
    "Identification",
    "Eye fixation task",
    "Temporal order judgment",
    "Free viewing",
    "Discrimination",
    "Detection",
    "Task is irrelevant to the effect",
    "Stem completion",
    "Counting",
    "Naming",
    "Memory task",
    "Change detection",
    "Solving an excersize",
]

uncon_stimulus_categories = {
    "Lingual": ["Words", "Letters", "Sentences", "Word Stems"],
    "Numerical": ["Digits", "Numbers", "Mathematical exercise"],
    "Pictures": ["Faces", "Objects", "Scenes", "Fruit", "Animals", "Vehicles", "Logos", "Hands", "Other (Pictures)"],
    "Shapes": ["Arrows", "Gratings", "Simple shapes", "Symbols", "Other (Shapes)"],
    "Line drawings": [
        "Faces (Line drawings)",
        "Objects (Line drawings)",
        "Fruit (Line drawings)",
        "Scenes (Line drawings)",
        "Animals (Line drawings)",
        "Vehicles (Line drawings)",
        "Logos (Line drawings)",
        "Hands (Line drawings)",
        "Other (Line drawings)",
    ],
    "Tactile Stimuli": [],
    "Biological motion": [],
    "Sounds": [],
}

uncon_stimulus_modalities = ["Auditory", "Olfactory", "Tactile", "Visual", "None"]

uncon_suppression_methods = {
    "Masking": [
        "Backward target masking",
        "Backward pattern masking",
        "Forward masking",
        "Metacontrast masking",
        "Object Substitution masking",
        "Paracontrast masking",
        "Sandwich target masking",
        "Sandwich pattern masking",
    ],
    "Dichoptic masking": [],
    "Dichotic fusion": [],
    "CFS": [],
    "Crowding": [],
    "Sub-threshold stimuli": [],
    "Attentional Blink": [],
    "Parafoveal display": [],
    "Stimulus degradation": ["Mooney Images", "Other"],
    "Inattentional blindness": [],
    "Binocular rivalry": [],
    "Change blindness": [],
    "Flickering": [],
}

uncon_processing_domains = [
    "Lingual",
    "Visual discrimination",
    "Numerical",
    "Memory",
    "Action priming",
    "Executive functions",
    "Working memory",
    "Attention allocation",
]

uncon_consciousness_measures_types = {
    "Objective": [
        "High-level discrimination",
        "Low-level discrimination",
        "Location",
        "Detection (presence/absence)",
        "Other",
    ],
    "Subjective": [
        "Dichotomous",
        "Perception Awareness Scale (PAS)",
        "Post-decision wagering",
        "Confidence ratings",
        "Continuous scale",
        "Likert Scale",
        "Other",
    ],
    "Verbal debriefing": [],
    "None": [],
    "Other": [],
}

uncon_consciousness_measures_phases = [
    "Post-experiment",
    "Pre-experiment",
    "Separate sample",
    "During the experiment, in alternating blocks",
    "Trial-by-trial",
    "None",
]

uncon_finding_outcomes = ["Reaction times", "Accuracy", "Eye movements", "Mouse trajectories", "Other"]
